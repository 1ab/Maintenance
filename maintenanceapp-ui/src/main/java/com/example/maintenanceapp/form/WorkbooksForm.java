...
import com.example.maintenanceapp.view.WorkbooksLogic;
 ...
 * CSS rules.
 */
public class WorkbooksForm extends JobsFormDesign {

	private WorkbooksLogic viewLogic;
	private Binder<Workbook> binder;
	private Workbook currentProduct;
	...
	public WorkbooksForm(WorkbooksLogic workbooksLogic) {
		super();
		addStyleName("product-form");
		viewLogic = workbooksLogic;
		...
		binder = new BeanValidationBinder<>(Workbook.class);
...
//		binder.bindInstanceFields(this);
		...
		save.addClickListener(event -> {
			if (currentProduct != null && binder.writeBeanIfValid(currentProduct)) {
				viewLogic.saveWorkbook(currentProduct);
			}
		});

		discard.addClickListener(event -> viewLogic.editWorkbook(currentProduct));

		cancel.addClickListener(event -> viewLogic.cancelWorkbook());

		delete.addClickListener(event -> {
			if (currentProduct != null) {
				viewLogic.deleteWorkbook(currentProduct);
			}
		});
	}
