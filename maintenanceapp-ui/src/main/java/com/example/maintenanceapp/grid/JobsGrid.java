 ...
 * data sets.
 */
public class JobsGrid extends Grid<Product> {
	...
	public JobsGrid() {
		...
		addColumn(this::htmlFormatAvailability, new HtmlRenderer()).setCaption("Availability")
				.setComparator((p1, p2) -> {
					return p1.getAvailability().toString().compareTo(p2.getAvailability().toString());
				});
	...
	private String formatCategories(Product product) {
		...
		return product.getCategory().stream().sorted(Comparator.comparing(Category::getId)).map(Category::getName)
